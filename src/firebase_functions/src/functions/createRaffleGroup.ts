...
import {
  CreateRaffleGroupRequestSchema,
  CreateRaffleGroupResponse,
  DrawStatusSchema,
  MemberRoleSchema,
  MemberStateSchema,
  RaffleStatusSchema
...
export const createRaffleGroup = onCall(
  async (req: CallableRequest<unknown>): Promise<CreateRaffleGroupResponse> => {
    try {
      const uid = requireAuthUid(req.auth?.uid);
      const body = parseOrThrow(CreateRaffleGroupRequestSchema, req.data);
        ...
        try {
          await db.runTransaction(async (tx) => {
            ...
            tx.create(groupRef, {
              ...
              raffleWinnerCount: body.raffleWinnerCount,
              ownerParticipatesInRaffle: body.ownerParticipatesInRaffle,
              lastRaffleExecutionId: null,
